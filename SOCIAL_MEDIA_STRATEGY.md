# Filtered Social Media Strategy

**Created:** 2026-01-20
**Updated:** 2026-02-24
**Status:** Active

---

## Brand Voice & Positioning

### Tone of Voice
- **Empathetic, not clinical** - We understand the struggle
- **Warm but not preachy** - Ally, not authority
- **Acknowledge difficulty first** - Then offer solutions
- **Use "we/you" language** - Position as companions in the journey
- **Gentle humor when appropriate** - Normalize the struggle

### Unique Positioning
**"Filtered = Your communication pause button"**
- First messaging app with emotional wellness integration
- Focus on communication improvement, not just documentation
- Target anyone dealing with hostile communication
- Build community, not just a tool

---

## Content Pillars (5)

### 1. Communication Tools & Tips (35%)
- Practical scripts for difficult conversations
- "What to say instead" reframes
- De-escalation techniques
- Boundary-setting language

### 2. Emotional Validation & Support (25%)
- "You're not alone" content
- Relatable struggles
- Mental health check-ins
- Self-compassion reminders

### 3. Practical Communication Wins (20%)
- Scheduling tips
- Documentation best practices
- User success stories
- Positive exchange celebrations

### 4. Healthy Boundaries Focus (10%)
- Setting and enforcing boundaries
- Recognizing manipulation patterns
- Self-care during difficult conversations

### 5. Product & Solution (10%)
- How Filtered solves problems
- Feature highlights
- App demos

---

## Content Mix Ratio

**70%** Value-driven (Educational + Emotional)
**20%** Community & Engagement
**10%** Promotional

---

## Platform Strategy

### Instagram (Primary)
- **Frequency:** 3-5 Reels + 1-2 Carousels + daily Stories weekly
- **Focus:** Educational carousels, relatable Reels, community Stories
- **Best times:** 7-9 AM, 7-9 PM (Tue-Thu)

### TikTok (Secondary)
- **Frequency:** 5-7 posts weekly
- **Focus:** Problem-solution videos, educational micro-series
- **Best times:** 5-9 PM (Mon-Thu), 8 PM Sunday

### Facebook (Tertiary)
- **Frequency:** 3-4 posts weekly
- **Focus:** Longer educational content, community building
- **Best times:** 11 AM Wed, 9 AM-12 PM Tue/Thu

---

## Weekly Posting Schedule

| Day | Instagram | TikTok | Facebook |
|-----|-----------|--------|----------|
| Monday | Story (motivational) | 6 PM (relatable) | - |
| Tuesday | 7 AM (carousel tip) | 5 PM (quick tip) | 10 AM (educational) |
| Wednesday | 7 AM Feed + 8 PM Reel | 8 PM (pillar content) | 11 AM (pillar content) |
| Thursday | 7 PM Reel (emotional) | 6 PM (storytelling) | - |
| Friday | 12 PM (lighter content) | - | - |
| Saturday | Stories (polls/engagement) | - | - |
| Sunday | - | 8 PM (high-value) | - |

---

## Content Series

1. **"What to Say Instead"** - Reframe hostile messages
2. **"3-Second Pause"** - Micro-reflection before responding
3. **"The Other Side"** - Perspective-shifting content
4. **"Communication Wins"** - Positive stories
5. **"Before You Send"** - Pause prompts

---

## CTAs by Stage

### Awareness (New followers)
- "Follow for more communication tips"
- "Save this for later"
- "Tag someone who needs this"

### Consideration (Engaged)
- "Link in bio to learn more"
- "Join our community"
- "Try Filtered free"

### Decision (Warm)
- "Download now - free trial"
- "Be first to know when we launch on iOS"
- "Get the app: link in bio"

---

## Hashtag Strategy

### Primary (use on every post)
#toxiccommunication #healthyboundaries #emotionalwellness

### Secondary (rotate)
#filteredmessaging #communicationtips #narcissistabuse
#mentalhealth #boundarieswork #difficultpeople
#manipulation #gaslighting #emotionalabuse

### Niche (use sparingly)
#highconflictrelationship #toxicex #hostilecommunication

---

## Metrics to Track

- Follower growth rate
- Engagement rate (likes + comments + shares / followers)
- Saves (indicates valuable content)
- Profile visits
- Link clicks (bio)
- App downloads attributed to social
